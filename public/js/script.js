	...
	$('#resetPassButton').on('click', function(event) {
        ...
        if($('#resPassInput').val() && $('#resPassInput').val().length >= 5){
            $.ajax({
                ...
                .done(function(resp) {
                    if(!resp.error){
                        window.location.pathname = '/admin/gorcakal/user-list';
                    }
...
//	show subregionsByRegion
    ...
    $("#addRealityAdmin").on("change", function(){
        $.ajax({
            url: '/admin/gorcakal/get-users/' + $(this).val() ,
            type: 'GET'
        })
            .done(function(data) {
                //subRegions
    ...
    $('.changeUserStatus').on('change', function(){
        var that = $(this)
        $.ajax({
            url: '/admin/gorcakal/update-user-status/' + $(this).siblings('input').val() +'/' + $(this).val() ,
            type: 'GET'
        })
            .done(function(data) {
                if(!data.error) {
